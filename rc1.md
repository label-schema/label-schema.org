...
| `docker.required-capabilities` | `org.label-schema.docker.required-capabilities = "SYS_TIME,NET_ADMIN,NET_BROADCAST"` | Required capabilities for the image to run under the Docker runtime. Multiple capabilities can be specified by separating with commas. |
